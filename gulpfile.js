...
gulp.task('serve', ['cp', 'sass:dev', 'sass:prod'], function() {
	var bsConfig = extend(true, {}, bsCfg, {
		files: [ sassCfg.devdest ],
		rewriteRules: [{
			match: new RegExp(themePath, 'g'),
			fn: function (match) {
				var path = sassCfg.devdest,
					startsWith = sassCfg.devdest.substr(0, 1);

				if (startsWith === '.') {
					path =  sassCfg.devdest.slice(1);
				}
				else if (startsWith !== '/') {
					path = '/' + sassCfg.devdest;
				}
...
gulp.task('sass:dev', function() {
    return gulp.src(sassCfg.src)
        ...
        .pipe(gulp.dest(sassCfg.devdest))
        .pipe(reload({ stream: true }));
});

// task for building breakpoint scss in variables
gulp.task('breakpoint', function () {
    return gulp.src(path.resolve(__dirname, 'adventure4life.breakpoints.yml'))
